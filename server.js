...
async function load() {
  ...
  const owner = db.users.find(
    u => String(u.username).toLowerCase() === "yung_marvelov"
  );
...
app.post("/api/register", async (req, res) => {
  try {
    ...
    if (
      db.users.some(
        u => u.phone && String(u.phone) === phone
      )
    ) {
      return res.status(400).json({
...
app.post(
  ...
  async (req, res) => {
    ...
    if (
      ...
      return res.status(400).json({
        error: "Потрібне зображення"
      });
    }

    req.user.avatar = img(req.file);
...
app.patch(
  ...
  async (req, res) => {
    ...
    if (!["", "check", "admin"].includes(badge)) {
      return res.status(400).json({
        error: "Невідомий значок"
      });
    }
