...
async function load(){
  ...
  if(!owner){
    ...
    process.env.RESET_OWNER_PASSWORD==="true"
  ){

    owner.password=
      await bcrypt.hash(desired,10);
...
app.post("/api/login",async(req,res)=>{
  ...
  const u=db.users.find(
    x=>
      x.username.toLowerCase()===login.toLowerCase()
      ||
      x.phone===login
  );
...
app.get("/api/rewards",auth,(req,res)=>{
  const completed=db.orders.filter(
    o=>
      String(o.userId)===String(req.user.id)
      &&
      o.status!=="Скасовано"
  );
  ...
  res.json([
    ...
    {
      title:"Придбати 1 товар",
      done:count>=1
    },
    {
      title:"Придбати 5 товарів",
      done:count>=5
    },
    {
      title:"Придбати 10 товарів",
      done:count>=10
    },
    {
      title:"Використати перший промокод",
      done:used
    }
  ]);
});
...
app.post("/api/promo/use",auth,async(req,res)=>{
  ...
  const p=db.promos.find(
    x=>x.code===code&&x.active!==false
  );
...
app.post("/api/orders",auth,async(req,res)=>{
  ...
  const ids=Array.isArray(b.items)
    ?b.items
    :[];
  ...
  if(
    ...
    return res.status(400).json({
      error:"Заповніть ім’я, прізвище та телефон"
    });
  }

  if(!["pickup","nova"].includes(b.delivery)){
    return res.status(400).json({
      error:"Оберіть доставку"
    });
  }
  ...
  if(
    ...
    return res.status(400).json({
      error:"Вкажіть населений пункт і відділення"
    });
  }
  ...
  if(b.promo){
    const p=db.promos.find(
      x=>
        x.code===
        String(b.promo).trim().toUpperCase()
        &&
        x.active!==false
    );
    ...
    if(p){
      ...
      if(!req.user.usedPromos.includes(p.code)){
        req.user.usedPromos.push(p.code);
      }
    }
  }
...
app.post(
  ...
  async(req,res)=>{
    ...
    if(req.body.name){
      p.name=String(req.body.name).trim();
    }
    ...
    if(req.body.desc!==undefined){
      p.desc=String(req.body.desc);
    }

    if(req.file){
      p.image=img(req.file);
    }
...
app.post(
  ...
  async(req,res)=>{
    ...
    if(req.user.role!=="owner"){
      return res.status(403).json({
        error:"Тільки головний адміністратор"
      });
    }
    ...
    if(
      ...
      return res.status(400).json({
        error:"Перевірте логін і пароль"
      });
    }
  ...
  async(req,res)=>{
    ...
    for(
      ...
      if(req.body[k]!==undefined){
        db.settings[k]=String(req.body[k]);
      }
    }
